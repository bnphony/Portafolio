export const experiencia = [
    {
        'cargo': 'Desarrollador Full-Stack',
        'empresa': 'Liga Deportiva de la Parroquia Mulaló',
        'fecha_inicio': 'Enero-2024',
        'fecha_fin': 'Marzo-2024',
        'actividades': [
            'Análisis y diseño del sistema.',
            'Toma de requisitos.',
            'Creación de Prototipos.',
            'Desarrollo de un Sistema para la Gestión de un Campeonato de Fútbol.',
            'Implementación del sistema en un Servidor Web.',
            'Generación de Reportes.'
        ],
        'tecnologias': ['Toma de Requisitos', 
            'Diseño de BD', 'Implementación', 
            'Python', 'Django', 'postgresql', 'HTML-CSS-JavaScript', 'JQuery', 'Ajax', 'Bootstrap'],
    },
    {
        'cargo': 'Técnico en Informática',
        'empresa': 'Sacompu',
        'fecha_inicio': 'Enero-2023',
        'fecha_fin': 'Junio-2023',
        'actividades': [
            'Soporte y Mantenimiento de Equipos de Cómputo y Redes.',
            'Implementación del cableado de redes informáticas.',
            'Gestión estratégica de inventarios.',
        ],
        'tecnologias': [
            'Microsoft Word', 
            'Microsoft Excel', 
            'Microsoft Power Point', 
            'Punto de Acceso',
            'Cableado',
            'Canvas',
            'Photoshop',
            'Ilustrator',
            'AnyDesk',
        ]
    }
];

export const proyectos = [
    {
        'nombre': 'Sistema para la Gestión de un Campeonato de Fútbol',
        'descripcion': `
            Campeonato de la Liga Deportiva de la Parroquia Mulaló, 
            gestionar los equipos, jugadores, categorias, copas, rondas, partidos, etc. Generar los calendarios, los partidos, generar 
            reportes de los resultados, etc.
        `,
        'tecnologias': ['Python', 'Django', 'postgresql', 'HTML-CSS-JavaScript', 'JQuery', 'Ajax', 'Bootstrap'],
        'imagen': 'img/proyecto_1.png',
        'url': 'https://campeonato-mulalo.top/',
        'github': '',
        'year': '2024',
    },
    {
        'nombre': 'Gestionar Ventas',
        'descripcion': `
            Aplicación Móvil sobre un Sistema de Venta de Productos.
        `,
        'tecnologias': ['Java', 'Android Studio', 'XML'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/Ventas-AS',
        'year': '2022',
    },
    {
        'nombre': 'Cronometro',
        'descripcion': `
            Aplicación Móvil para programar eventos y controlar el tiempo para que ocurran.
        `,
        'tecnologias': ['Java', 'Android Studio', 'XML'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/Cronometro-AS',
        'year': '2023',
    },
    {
        'nombre': 'Calificar Series',
        'descripcion': `
            Sistema para la Valoración de Series/Películas, además de presentar reportes.
        `,
        'tecnologias': ['PHP', 'Mysql', 'HTML-CSS-JavaScript', 'D3js'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/Calificar-Series',
        'year': '2023',
    },
    {
        'nombre': 'Suma Mental',
        'descripcion': `
            Sistema para ejercitar la resolución de operaciones matemáticas (suma/resta) mentalmente.
        `,
        'tecnologias': ['python', 'django', 'jquery', 'd3js', 'HTML-CSS-JavaScript'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/Suma-Mental',
        'year': '2024',
    },
    {
        'nombre': 'Lista Comentada',
        'descripcion': `
            Aplicación Web para convertir una lista simple en un ranking ordenado.
        `,
        'tecnologias': ['HTML-CSS-JavaScript', 'JQuery'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/ListaComentada',
        'year': '2024',
    },
    {
        'nombre': 'Sistema de Inventario',
        'descripcion': `
            Sistema de Inventarios y generación de reportes.
        `,
        'tecnologias': ['python', 'django', 'HTML-CSS-JavaScript'],
        'imagen': '',
        'url': '',
        'github': 'https://github.com/bnphony/Sistema-Inventario',
        'year': '2022',
    },
];
